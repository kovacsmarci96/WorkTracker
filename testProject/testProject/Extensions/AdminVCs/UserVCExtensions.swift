...
extension UserVC : UIPickerViewDelegate {
}

extension UserVC: UIPickerViewDataSource {
...
extension UserVC {
    ...
    func fetchTasks(_ semaphore: DispatchSemaphore,_ filter: String) {
        for project in projects {
            getAllTask(project.id!, loggedInUser.token!, semaphore)
            semaphore.wait()
            for task in tasks {
    ...
    func getAllWork(_ projId: String,_ taskId: String,_ filter: String) {
        ...
        workRequest.getAllWork(loggedInUser.token!, completion: {result in
            switch result{
